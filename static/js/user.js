/* user.js — modular, robust, responsive-ready
   - localStorage CRUD: ft_user_active, ft_user_archive
   - responsive Chart.js usage
   - safe DOM guards & debounce for filter
*/
...
const dom = {
  ...
  date: $('#dateUser'),
  tableBody: $('#tableBodyUser'),
  archiveList: $('#archiveListUser'),
  avgCal: $('#avgCalUser'),
  avgDur: $('#avgDurUser'),
  freqAct: $('#freqActUser'),
  longest: $('#longestUser'),
  calBar: $('#calBarUser'),
  workBar: $('#workBarUser'),
  exportCSV: $('#exportCSVUser'),
  exportJSON: $('#exportJSONUser'),
  importFile: $('#importFileUser'),
  clearArchive: $('#clearArchiveUser'),
  restoreAll: $('#restoreAllUser'),
  logoutBtn: $('#logoutBtn'),
  filterInput: $('#filter'),
...
/* Defaults */
if (dom.date) dom.date.value = new Date().toISOString().slice(0,10);
...
function renderTable(filter='') {
  ...
  // bind actions
  $$('.btn-delete').forEach(b => b.onclick = () => deleteWorkout(b.dataset.id));
  $$('.btn-archive').forEach(b => b.onclick = () => archiveWorkout(b.dataset.id));
}
...
function renderArchive() {
  ...
  $$('.btn-restore').forEach(b => b.onclick = () => restoreArchived(b.dataset.id));
  $$('.btn-del-arch').forEach(b => b.onclick = () => {
...
function renderCharts() {
  ...
  // Calories line
  const calCanvas = document.getElementById('caloriesChartUser');
  if (calCanvas) {
    ...
    if (!calChart) {
      calChart = new Chart(calCtx, {
        ...
        options: { responsive: true, maintainAspectRatio: false, plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true}}, animation:{duration:400} }
      });
    } else {
      calChart.data.labels = labels; calChart.data.datasets[0].data = agg.totalsCal; calChart.update();
    }
  }

  // Duration bar
  const durCanvas = document.getElementById('durationChartUser');
  if (durCanvas) {
    const durCtx = durCanvas.getContext('2d');
    if (!durChart) {
      ...
      durChart.data.labels = labels; durChart.data.datasets[0].data = agg.totalsDur; durChart.update();
    }
  }
...
function updateStats(agg) {
  ...
  dom.avgCal && (dom.avgCal.textContent = `${Math.round(totalCal/7)||0} cal/day`);
  ...
  dom.avgDur && (dom.avgDur.textContent = `${avgDur} min`);

  const freq = {}; combined.forEach(w => freq[w.activity] = (freq[w.activity]||0) + 1);
  const freqAct = Object.keys(freq).length ? Object.entries(freq).sort((a,b)=>b[1]-a[1])[0][0] : '-';
  dom.freqAct && (dom.freqAct.textContent = freqAct);

  const longest = combined.slice().sort((a,b)=>b.duration-a.duration)[0];
  dom.longest && (dom.longest.textContent = longest ? `${longest.activity} — ${longest.duration} min` : '-');
  ...
  if (dom.calBar) { dom.calBar.style.width = calPct + '%'; dom.calBar.textContent = calPct + '%'; }
  if (dom.workBar) { dom.workBar.style.width = workPct + '%'; dom.workBar.textContent = workPct + '%'; }

  // Toggle logout visibility
  if (dom.logoutBtn) dom.logoutBtn.style.display = (active.length || read(KEY_ARCHIVE).length) ? 'inline-flex' : 'none';
}
...
if (dom.addForm) {
  dom.addForm.addEventListener('submit', e => {
    ...
    if (!item.activity || item.duration <= 0 || item.calories <= 0) {
      return alert('Enter valid values');
    }
    addWorkout(item);
    // reset values
    if (dom.activity) dom.activity.value = '';
    if (dom.duration) dom.duration.value = 30;
    if (dom.calories) dom.calories.value = 300;
    if (dom.date) dom.date.value = new Date().toISOString().slice(0,10);
  });
}

/* Export CSV/JSON & Import */
if (dom.exportCSV) dom.exportCSV.addEventListener('click', () => {
  ...
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'workouts.csv'; a.click(); URL.revokeObjectURL(url);
});
if (dom.exportJSON) dom.exportJSON.addEventListener('click', () => {
  const arr = read(KEY_ACTIVE);
  const blob = new Blob([JSON.stringify(arr,null,2)], { type:'application/json' });
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'workouts.json'; a.click(); URL.revokeObjectURL(url);
});
if (dom.importFile) dom.importFile.addEventListener('change', e => {
  const file = e.target.files?.[0]; if (!file) return;
  const reader = new FileReader();
  reader.onload = () => {
    try {
      const arr = JSON.parse(reader.result);
      if (Array.isArray(arr)) write(KEY_ACTIVE, arr);
      renderAll();
    } catch (err) {
...
if (dom.clearArchive) dom.clearArchive.addEventListener('click', () => {
  ...
  write(KEY_ARCHIVE, []); renderAll();
});
if (dom.restoreAll) dom.restoreAll.addEventListener('click', () => {
  const arch = read(KEY_ARCHIVE); const act = read(KEY_ACTIVE);
  write(KEY_ACTIVE, act.concat(arch)); write(KEY_ARCHIVE, []); renderAll();
});

/* Logout */
if (dom.logoutBtn) dom.logoutBtn.addEventListener('click', () => {
  if (!confirm('Are you sure you want to logout?')) return;
  localStorage.removeItem(KEY_ACTIVE); localStorage.removeItem(KEY_ARCHIVE); localStorage.removeItem('user_avatar');
...
const doFilter = debounce(() => {
  const v = dom.filterInput?.value?.trim() || '';
  renderAll(v);
});
if (dom.applyFilter) dom.applyFilter.addEventListener('click', () => doFilter());
if (dom.clearFilter) dom.clearFilter.addEventListener('click', () => { if (dom.filterInput) dom.filterInput.value=''; renderAll(); });
if (dom.filterInput) dom.filterInput.addEventListener('input', () => doFilter());

/* Ensure charts resize on container changes */
window.addEventListener('resize', () => {
  calChart?.resize?.(); durChart?.resize?.();
});
