...
const workouts = read(KEY_ACTIVE).slice().sort((a,b)=> b.date.localeCompare(a.date));
...
dom.tableBody.innerHTML = filtered.map(w => `     <tr>       <td>${isoToDisplay(w.date)}</td>       <td>${w.activity}</td>       <td>${safeNum(w.duration)} min</td>       <td>${safeNum(w.calories)}</td>       <td class="text-end">         <div class="btn-group btn-group-sm">           <button class="btn btn-outline-danger btn-delete" data-id="${w.id}"><i class="fa-solid fa-trash"></i></button>           <button class="btn btn-outline-secondary btn-archive" data-id="${w.id}"><i class="fa-solid fa-box-archive"></i></button>         </div>       </td>     </tr>
...
dom.archiveList.innerHTML = arch.map(w => `     <li class="list-group-item d-flex justify-content-between align-items-start">       <div>         <strong>${w.activity}</strong>         <div class="small-muted">${isoToDisplay(w.date)} • ${safeNum(w.duration)} min • ${safeNum(w.calories)} cal</div>       </div>       <div class="btn-group">         <button class="btn btn-sm btn-outline-success btn-restore" data-id="${w.id}"><i class="fa-solid fa-rotate-left"></i></button>         <button class="btn btn-sm btn-outline-danger btn-del-arch" data-id="${w.id}"><i class="fa-solid fa-trash"></i></button>       </div>     </li>
...
if(!calChart){
calChart = new Chart(document.getElementById('caloriesChartUser').getContext('2d'), {
type:'line',
...
if(!durChart){
durChart = new Chart(document.getElementById('durationChartUser').getContext('2d'), {
type:'bar',
...
dom.avgCal.textContent = `${Math.round(combined.reduce((s,w)=>s+w.calories,0)/7)||0} cal/day`;
...
const arr = read(KEY_ACTIVE); const blob = new Blob([JSON.stringify(arr,null,2)],{type:'application/json'});
...
const file=e.target.files[0]; if(!file) return;
const reader=new FileReader();
reader.onload=()=>{ try{ const arr=JSON.parse(reader.result); if(Array.isArray(arr)) write(KEY_ACTIVE, arr); renderAll(); }catch{ alert('Invalid JSON file'); } };
...
// ------------------- LOGOUT -------------------
dom.logoutBtn.style.display = (localStorage.getItem(KEY_ACTIVE) || localStorage.getItem(KEY_ARCHIVE)) ? 'inline-flex' : 'none';
